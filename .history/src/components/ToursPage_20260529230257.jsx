...
export default function ToursPage() {
  ...
  const filteredTours = useMemo(() => {
    ...
    return tours.filter((t) => {
      ...
      if (fFrom || fTo) {
        const hasMatch = (t.availableDates || []).some((d) => {
          const dt = new Date(d);
          if (fFrom && dt < fFrom) return false;
          if (fTo && dt > fTo) return false;
          return true;
        });
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      // only capture wheel when pointer is over the deck
  ...
  return (
    <section className="tours-page page" id="tours">
      ...
      <div
        ...
        onPointerEnter={() => setIsOverDeck(true)}
        onPointerLeave={() => setIsOverDeck(false)}
      >
