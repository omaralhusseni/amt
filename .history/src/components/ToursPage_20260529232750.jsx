...
export default function ToursPage() {
  ...
  return (
    <section id="tours" style={{ background: 'var(--sand-light)' }} className="min-h-screen pt-24 relative">
      <div className="flex items-end justify-between" style={{ padding: '40px var(--page-pad) 60px' }}>
        <div>
          <h2 className="tours-heading">
            Our <span>Journeys</span>
          </h2>
          <p className="mt-3 text-sm text-[var(--text-muted)]" style={{ letterSpacing: '0.05em' }}>
            Scroll through our curated collection
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex flex-col min-w-[200px]">
            <label className="text-xxs text-amber-400 uppercase tracking-wider mb-1">Destination</label>
            <DestinationSelect options={DEST_OPTIONS} selected={selectedDestinations} onChange={setSelectedDestinations} buttonClass="px-3 py-2 rounded bg-white" dropdownZ={99999} />
          </div>
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <label className="text-xxs text-[var(--text-muted)]">From</label>
              <input className="px-3 py-2 rounded" type="date" value={from} onChange={(e) => setFrom(e.target.value)} />
            </div>
            <div className="flex flex-col">
              <label className="text-xxs text-[var(--text-muted)]">To</label>
              <input className="px-3 py-2 rounded" type="date" value={to} onChange={(e) => setTo(e.target.value)} />
            </div>
            <div className="flex flex-col">
              <label className="text-xxs text-[var(--text-muted)]">Guests</label>
              <input className="px-3 py-2 rounded w-20" type="number" min="1" value={guests} onChange={(e) => setGuests(e.target.value)} />
            </div>
            <div className="flex items-end">
              <button className="border border-amber-400 text-amber-400 px-4 py-2 rounded" onClick={() => {
      ...
      <div ref={containerRef} className="relative" style={{ marginLeft: 'var(--page-pad)', marginRight: 'var(--page-pad)', height: 'min(70vh,680px)' }}>
