...
export default function ToursPage() {
  ...
  useEffect(() => {
    const onDoc = (e) => {
      ...
      if (!destRef.current.contains(e.target)) setDestOpen(false);
    };
    ...
    <section className="tours-page page" id="tours">
      <div className="tours-header fade-up fade-up-1">
        ...
        <div className="filter-bar">
          <div className="date-input" style={{ minWidth: 200 }} ref={destRef}>
            <label className="search-label">Destination</label>
            <div style={{ position: 'relative', width: '100%' }}>
              <button className="booking-input" style={{ textAlign: 'left' }} onClick={() => setDestOpen((s) => !s)}>
                {selectedDestinations.length === 0 ? 'Any destination' : selectedDestinations.join(', ')}
              </button>
              {destOpen && (
                <div style={{ position: 'absolute', zIndex: 40, top: '110%', left: 0, right: 0, background: 'white', borderRadius: 10, padding: 12, boxShadow: '0 10px 30px rgba(0,0,0,0.12)', color: 'var(--dark)' }}>
